...
namespace tmdb_file_rename
{
    class Program
    {
        ...
        {
            //TODO: Keeping the arguments in so I don't have to enter the api key while testing
            if (args.Length < 2)
            {
            ...
            else
            {
                ...
                DirectoryInfo directory = new DirectoryInfo(@"E:\RenameTest");
                ...
                if (response == ConsoleKey.Y)
                {
                    ...
                    //TODO: Create a text file with all the skipped filesnames to keep track of
                    // what needs to be manually renamed
                    //TextWriter tw = new StreamWriter("TMDB-skipped-files.txt");
            ...
            foreach (FileInfo file in files)
            {
                ...
                else
                {
                    Console.WriteLine("Mutiple results for: {0}\n" +
                        ...
                        "----------------------------------------", file.FullName);
                    selectedNames.Add(NameSelectionMenu(movieNames, Path.GetFileNameWithoutExtension(file.FullName)));
                }
            }
            return Tuple.Create(selectedNames,skippedFiles);
        }

        public static string NameSelectionMenu(List<string> movieNameOptions, string originalName)
        {
            ...
            return selectionLookup[selectionInput];
        }
